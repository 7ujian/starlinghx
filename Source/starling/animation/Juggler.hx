...
class Juggler implements IAnimatable
{
	...
	public function add(object:IAnimatable):Void
	{
		if (object != null && mObjects.indexOf(object) == -1)
		{
			...
			var dispatcher = cast(object, EventDispatcher);
	...
	public function containsTweens(target:Dynamic):Bool
	{
		...
		while(i>=0)
		{
			var tween = cast(mObjects[i], Tween);
	...
	public function purge():Void
	{
		...
		// to 'advanceTime'.
		var i = mObjects.length;
		while (i >= 0)
		{
			var dispatcher = cast (mObjects[i], EventDispatcher);
			if (dispatcher != null) dispatcher.removeEventListener(Event.REMOVE_FROM_JUGGLER, onRemove);
			mObjects[i] = null;
			i--;
		}
	...
	public function delayCall(call:Array<Dynamic>->Void, delay:Float, args:Array<Dynamic>):DelayedCall
	{
	...
	public function tween(target:Dynamic, time:Float, properties:Dynamic):Void
	{
		...
		for (property in Reflect.fields(properties))
		{
			...
			if (Reflect.hasField(tween, property))
				Reflect.setField(tween, property, value);
...
//		}
	...
	public var elapsedTime(get_elapsedTime, null):Float;
