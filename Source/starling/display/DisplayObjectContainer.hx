...
//		}
			...
			else
			{
				...
				if (stage !=null)
				{
					if (Std.is(child,  DisplayObjectContainer))
						cast(child, DisplayObjectContainer).broadcastEventWith(Event.ADDED_TO_STAGE);
					else
	...
	public function removeChildAt(index:Int, dispose:Bool=false):DisplayObject
	{
		if (index >= 0 && index < numChildren)
		{
			...
			{
				if (Std.is(child, DisplayObjectContainer))
				 	cast(child, DisplayObjectContainer).broadcastEventWith(Event.REMOVED_FROM_STAGE);
				else           child.dispatchEventWith(Event.REMOVED_FROM_STAGE);
			}

			child.setParent(null);
			index = Lambda.indexOf(mChildren, child); // index might have changed by event handler
	...
	{
		return Lambda.indexOf(mChildren, child);
	}
	...
	/** @private */
	@:allow(starling.display) function getChildEventListeners(object:DisplayObject, eventType:String,
