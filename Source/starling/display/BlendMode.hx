...
class BlendMode
{
	private static var sBlendFactors = [
	...
	{
		var index = premultipliedAlpha ? 1 : 0;
		var modes = sBlendFactors[index];
		return modes.get(mode);
		//if (modes.exists(mode)) return Reflect.getProperty(modes, mode);
	...
	{
		var index = premultipliedAlpha ? 1 : 0;
		var modes:Dynamic = sBlendFactors[index];
		Reflect.setProperty(modes, name, [sourceFactor, destFactor]);

		var otherModes:Dynamic = sBlendFactors[(!premultipliedAlpha)?1:0];
		if (!(Reflect.hasField(otherModes, name))) Reflect.setProperty(otherModes, name, [sourceFactor, destFactor]);
	}
}
