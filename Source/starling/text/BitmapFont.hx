...
class BitmapFont
{
	...
	private var mChars:IntMap<BitmapChar>;
	...
	private function parseFontXml(fontXml:Xml):Void
	{
		...
		var fast = new Fast(fontXml);

		mName = fast.node.info.att.face;
		mSize = Std.parseFloat(fast.node.info.att.size) / scale;
		if (mSize<=0) throw(new Error());
		mLineHeight = Std.parseFloat(fast.node.common.att.lineHeight) / scale;
		mBaseline = Std.parseFloat(fast.node.common.att.base) / scale;

		if (fast.node.info.att.smooth == "0")
		...
		for (charElement in fast.node.chars.nodes.char)
		{
			var id:Int = Std.parseInt(charElement.att.id);
			var xOffset:Float = Std.parseFloat(charElement.att.xoffset) / scale;
			var yOffset:Float = Std.parseFloat(charElement.att.yoffset) / scale;
			var xAdvance:Float = Std.parseFloat(charElement.att.xadvance) / scale;

			var region:Rectangle = new Rectangle();
			region.x = Std.parseFloat(charElement.att.x) / scale + frame.x;
			region.y = Std.parseFloat(charElement.att.y) / scale + frame.y;
			region.width  = Std.parseFloat(charElement.att.width) / scale;
			region.height = Std.parseFloat(charElement.att.height) / scale;
		...
		for (kerningElement in fast.node.kernings.nodes.kerning)
		{
			var first:Int = Std.parseInt(kerningElement.att.first);
			var second:Int = Std.parseInt(kerningElement.att.second);
			var amount:Float = Std.parseFloat(kerningElement.att.amount) / scale;
								 ...
								 kerning:Bool=true):Sprite
	{
		...
		fillSprite(sprite, width, height, text, fontSize, color, hAlign, vAlign, autoScale, kerning);

		return sprite;
	}

	/** Fill a sprite that contains a certain text, made up by one image per char. */
	public function fillSprite(sprite:Sprite, width:Float, height:Float, text:String,
								 fontSize:Float=-1, color:UInt=0xffffff,
								 hAlign:String="center", vAlign:String="center",
								 autoScale:Bool=true,
								 kerning:Bool=true):Void
	{
		var charLocations = arrangeChars(width, height, text, fontSize,
	...
	{
		if (text == null || text.length == 0) return new Vector<CharLocation>();
		if (fontSize < 0) fontSize *= -mSize;

		var lines:Vector<Vector<CharLocation>> = new Vector<Vector<CharLocation>>();
	...
	public var lineHeight(get_lineHeight, set_lineHeight):Float;
	...
	public var baseline(get_baseline, set_baseline):Float;
